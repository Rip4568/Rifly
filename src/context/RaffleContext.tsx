...
const raffleReducer = (state: UserState, action: RaffleAction): UserState => {
  switch (action.type) {
    ...
    case 'BUY_TICKET':
      return {
        ...state,
        balance: state.balance - 0, // Mock: user buying doesn't deduct from *creator* balance, but we could simulate income if needed.
        // For simplicity, let's just mark the ticket as sold
        raffles: state.raffles.map((r) => {
          if (r.id === action.payload.raffleId) {
             if (r.soldTickets.includes(action.payload.ticketNumber)) return r;
             return { ...r, soldTickets: [...r.soldTickets, action.payload.ticketNumber] };
          }
          return r;
        }),
      };
...
interface RaffleContextType {
  ...
  createRaffle: (raffle: Omit<Raffle, 'id' | 'createdAt' | 'soldTickets'>) => void;
  deleteRaffle: (id: string) => void;
  editRaffle: (raffle: Raffle) => void;
  buyTicket: (raffleId: string, ticketNumber: number) => void;
}

const RaffleContext = createContext<RaffleContextType | undefined>(undefined);

export const RaffleProvider = ({ children }: { children: ReactNode }) => {
  const [state, dispatch] = useReducer(raffleReducer, initialState, (initial) => {
  ...
  const createRaffle = (data: Omit<Raffle, 'id' | 'createdAt' | 'soldTickets'>) => {
  ...
  const buyTicket = (raffleId: string, ticketNumber: number) => {
    dispatch({ type: 'BUY_TICKET', payload: { raffleId, ticketNumber } });
  };

  return (
    <RaffleContext.Provider value={{ state, dispatch, createRaffle, deleteRaffle, editRaffle, buyTicket }}>
      {children}
